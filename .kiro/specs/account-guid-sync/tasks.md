...
- [ ] 1. Add AccountId to client data model and repository
  - [ ] 1.1 Add `AccountId` property to client `AccountDTO`
  ...
  - [ ] 1.2 Implement `GetByAccountIdAsync` in `AccountRepo`
  ...
  - [ ] 1.3 Write property test for client repository lookup
...
- [ ] 2. Add AccountId to client API models
  - [ ] 2.1 Add `AccountId` to `AccountReq`
    - Add `public Guid? AccountId { get; set; }` to `AccountReq` in `Model/Req/AccountReq.cs`
    - _Requirements: 3.1, 3.4_

  - [ ] 2.2 Add `AccountId` to `AccountApiRes`
    - Add `public Guid? AccountId { get; set; }` to `AccountApiRes` in `Model/Res/Api/AccountApiRes.cs`
    - _Requirements: 1.3, 5.1_

- [ ] 3. Add AccountId to server data model and repository
  - [ ] 3.1 Add `AccountId` property to server `AccountDTO`
  ...
  - [ ] 3.2 Create EF Core migration for AccountId column
  ...
  - [ ] 3.6 Write property test for server repository lookup
...
- [ ] 5. Implement server upsert logic
  - [ ] 5.1 Modify server `AccountService.CreateAsync` to implement upsert by AccountId
  ...
  - [ ] 5.2 Map `AccountId` in existing account response (GET endpoint)
    - Update the mapping in the server's account retrieval logic to include `AccountId` in responses
    - _Requirements: 2.4_

  - [ ] 5.3 Write property test for server upsert idempotence
  ...
  - [ ] 5.4 Write property test for server Guid generation when none provided
...
- [ ] 6. Implement client push logic with AccountId
  - [ ] 6.1 Modify `AccountService` push to include AccountId in request
  ...
  - [ ] 6.2 Modify `CreateAsync` to assign `Guid.NewGuid()` to AccountId
  ...
  - [ ] 6.3 Modify `EnsureDefaultAccountAsync` to assign `Guid.NewGuid()` to AccountId
    - Apply same Guid assignment logic when creating the default account
    - _Requirements: 10.4_

  - [ ] 6.4 Ensure `UpdateAsync` does NOT overwrite existing AccountId
    - When updating an account locally, the service SHALL NOT change a non-empty `AccountId` to a different value
    - _Requirements: 10.5_

  - [ ] 6.5 Write property test for Guid assignment on creation
  ...
  - [ ] 6.6 Write property test for push round-trip identity preservation
  ...
  - [ ] 6.7 Write property test for push failure leaves record unchanged
  ...
  - [ ] 6.8 Write property test for AccountId immutability after assignment
...
- [ ] 7. Implement client pull logic with AccountId matching
  - [ ] 7.1 Modify `PullAsync` to match by AccountId first
  ...
  - [ ] 7.2 Write property test for pull AccountId matching with last-writer-wins
  ...
  - [ ] 7.3 Write property test for pull inserting new records
